...
def default_phase_payloads(phase: str, page: str) -> dict[str, Any]:
    ...
    if suffix == "scout":
        return {
            ...
            "scout_rejections.json": {"phase": phase, "page": page, "generatedAt": now, "rejections": [], "defaultBias": "prefer software/service/engine/technology companies over pure hardware vendors and evaluate the full global pool except South Korea/China headquarters"},
            "reserve_candidates.json": {"phase": phase, "page": page, "generatedAt": now, "reserve": [], "reasons": ["Reject or reserve South Korea/China HQ, unicorns, unclear HQ, and pure hardware-first vendors without service/engine leverage. Search the full eligible global pool and rank by evidence strength."]},
...
def bootstrap_phase(phase: str, page: str, role: str, run_date: str) -> Path:
    ...
    context = "\n".join(
        [
            ...
            "- 우대 규칙: HW 단독 업체보다 SW·서비스·엔진·기술형 회사를 우대하고, 한국·중국 본사만 제외한 글로벌 전체 후보군에서 근거가 가장 강한 회사를 먼저 본다.",
