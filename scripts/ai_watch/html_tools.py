...
BULLET_STYLE_ISSUE_RE = re.compile(
    r"(?:[.。]\s*|한다|된다|있다|없다|좋다|높다|크다|맞다|필요하다|가능하다|어렵다|쉽다|커진다|늘어난다|이어진다|형성된다|생긴다|보여\s*준다|봐야\s*한다|해야\s*한다|보수적이어야\s*한다|제공한다|판매한다|강화한다|확장한다|연결한다|배포한다|고도화한다|활용한다|자동화한다|통합한다|구축한다|단축한다|가속한다|통제한다|탐색한다|실험한다|작동한다|절제한다|뒷받침한다|설계한다|전환한다|집행한다|처리한다)\s*$"
)
