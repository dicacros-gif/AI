...
4. current `1/index.html`
5. same-run approved AI/1 artifacts

Goal:
- Render AI/1 conservatively with minimal HTML diff
...
- Normalize awkward fragment endings before publish, for example `단단하 → 단단`, `다시 핵심으로 올라왔 → 다시 핵심`, `재편됐 → 재편`, `신호를 준 → 신호를 줌`, `두꺼워졌 → 명확`, `선언했 → 선언`, `강화했 → 강화`, and never leave orphaned sentence-final `'다`
